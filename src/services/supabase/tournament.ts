...
export const TournamentService = {
  ...
  generateTournamentStructure: async (
    ...
    try {
        ...
        if (!tournamentId) {
            const settings = { groupSize: options?.groupSize || 4, qualifiersPerGroup: 2 }; // Example default settings
        ...
        const groupSize = options?.groupSize || 4; // Default group size
        const numGroups = Math.ceil(teams.length / groupSize);
        const shuffledTeams = TournamentService.shuffleArray(teams); // Use service's shuffle
