...
export const TournamentBracket: React.FC<TournamentBracketProps> = ({ eventId }) => {
  ...
  const matchesByStage = useMemo(() => {
    ...
    return uniqueMatches.reduce((acc, match) => {
      if (match.stage === 'GROUP') {
        ...
        // Debug para partidas de eliminação
        console.log(`🏆 [DEBUG] Elimination match found:`, {
          id: match.id,
          round: match.round,
          position: match.position,
          stage: match.stage,
          team1Count: match.team1?.length || 0,
          team2Count: match.team2?.length || 0,
  ...
  // 🔍 DEBUG CRÍTICO: Log das partidas de eliminação após filtragem
  console.log(`🔍 [DEBUG CRÍTICO] eliminationMatches após filtragem:`, eliminationMatches.length);
  console.log(`🔍 [DEBUG CRÍTICO] eliminationMatches por rodada:`, 
    eliminationMatches.reduce((acc, match) => {
      acc[match.round || 0] = (acc[match.round || 0] || 0) + 1;
      return acc;
  ...
  const { eliminationRoundsArray, bracketLines, matchPositionMap } = useMemo(() => {    
    ...
    // 🔍 DEBUG CRÍTICO: Log de todas as partidas de eliminação
    console.log(`🔍 [DEBUG CRÍTICO] eliminationMatches recebidas:`, eliminationMatches.length);
    eliminationMatches.forEach((match, index) => {
      console.log(`   ${index + 1}. Match ID: ${match.id}, Round: ${match.round}, Position: ${match.position}, Team1: ${match.team1?.length || 0}, Team2: ${match.team2?.length || 0}`);
    });
    ...
    const determineSide = (match: Match): 'left' | 'right' | 'final' => {
      ...
      // Check if this is the final match (highest round with only one match)
      const maxRound = Math.max(...eliminationMatches.map(m => m.round || 0));
      const matchesInMaxRound = eliminationMatches.filter(m => m.round === maxRound).length;
      
      if (match.round === maxRound && matchesInMaxRound === 1) {
        return 'final';
      }
      
      // CORREÇÃO CRÍTICA: Para visualização em tabela, simplificar distribuição
      // Como estamos mostrando em formato de tabela (não gráfico bilateral),
      // vamos colocar todas as partidas no 'left' para simplificar
      // e focar na organização correta por rodadas
    ...
    eliminationMatches.forEach(match => {
      ...
      // Debug: Log detalhado das partidas
      console.log(`🏆 [DEBUG] Match ${match.id}: round=${match.round}, position=${match.position}, side=${side}, team1=${match.team1?.length}, team2=${match.team2?.length}`);
    ...
    // Debug: Log da organização final
    console.log(`🏆 [DEBUG] Left side rounds:`, Object.keys(leftSideMatches).map(Number).sort());
    console.log(`🏆 [DEBUG] Right side rounds:`, Object.keys(rightSideMatches).map(Number).sort());
    console.log(`🏆 [DEBUG] Final rounds:`, Object.keys(finalMatches).map(Number).sort());
  // Function to position matches with proper horizontal center alignment and vertical spacing
    ...
    // Instead of separating by sides, group all matches by their actual round number
    const allRoundsMap = new Map<number, Match[]>();
    
    // Add matches from all sides to their respective rounds
    Object.entries(leftSideMatches).forEach(([round, matches]) => {
      const roundNum = parseInt(round);
      if (!allRoundsMap.has(roundNum)) {
        allRoundsMap.set(roundNum, []);
      }
      allRoundsMap.get(roundNum)!.push(...matches);
    });
    ...
    Array.from(allRoundsMap.entries())
      .sort(([a], [b]) => a - b)
    ...
    // Debug detalhado da organização das rodadas
    console.log(`🏆 [DEBUG] Eliminação - Total de partidas:`, eliminationMatches.length);
    console.log(`🏆 [DEBUG] Eliminação - Partidas por rodada:`, 
      eliminationMatches.reduce((acc, match) => {
        acc[match.round || 0] = (acc[match.round || 0] || 0) + 1;
        return acc;
      }, {} as Record<number, number>)
    );
    console.log(`🏆 [DEBUG] Eliminação - RoundsArray final:`, 
      roundsArray.map(r => ({ 
        round: r.round, 
    ...
    console.log(`🏆 [Bracket Organization] Total rounds: ${roundsArray.length}`, 
                roundsArray.map(r => ({ round: r.round, matches: r.matches.length })));
